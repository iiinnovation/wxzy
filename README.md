...
```bash
curl -s -X POST "http://127.0.0.1:8000/admin/cards/import-seed" \
...
curl -s http://127.0.0.1:8000/stats/summary -H "Authorization: Bearer dev-token-change-me"
```

以上命令也可作为最小冒烟流程：导入后访问 `/review/due`，调用 `/review/answer`（`rating` 为 1 到 4），再次访问 `/review/due` 和 `/stats/summary`，确认 due 与复习统计发生变化。当前 `fsrs-v1` 为可升级的轻量 MVP 调度器，生产环境上线前需用标准 FSRS 参数实现替换并重新评测。
...
2. 填入你的 AppID（替换 `project.config.json` 里的 `touristappid`）
3. 开发阶段可关闭「校验合法域名」
4. 在「我的」页设置：
...
```bash
curl -s -X POST "http://127.0.0.1:8000/admin/cards/import" \
  -H "Authorization: Bearer dev-token-change-me" \
