...
数据库唯一约束为 `(user_id, card_id)`。新发布卡或 catalog card 的创建不产生 enrollment；
加入学习只产生 `queued` 行，不产生复习状态。计划首次引入时，enrollment 转为 `active` 并
原子创建对应的 CardReviewState；重复加入/引入返回现有行，不创建重复状态。`suspended` 和
`retired` 只改变 enrollment 可计划性，不删除任何复习状态或作答历史。
...
CardReviewState 的唯一键是 `(user_id, card_id)`，并通过同样的 user/card 外键保存个人归属。
迁移过渡期保留旧的无 user_id `review_states` 供兼容 API 使用；P1-T05 才将 legacy 行对账
到新表。默认新状态为 `new`、`due_at=引入时间`、稳定性 1.0、难度 5.0、`fsrs-v1`，这些
只是迁移期间的保守初值，不代表已完成标准 FSRS 评测。
