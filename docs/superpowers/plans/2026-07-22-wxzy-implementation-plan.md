...
完成记录：
- 完成提交：`0b8aeb6`。
- Job `wave0_p4t01_v1`：7 文件 / 70 页；MinerU vlm+ocr+ch；上传/解析/下载完成。
- clean.v2 / structure.v1 / quality.v1 已跑通；7/7 明确 `needs_review`（raw 水印 OCR 可洗；clean 后 residual=0）。
- 冻结参数：`model_version=vlm`, `language=ch`, `is_ocr=true`, formula+table on；batch 7×10 页。
- 报告：本地 `data/document-pipeline/wave0/wave0-report.v1.{json,md}`；tracked 摘要 `docs/superpowers/reports/2026-07-24-wave0-p4t01.{json,md}`（无全文）。
- 配额占用：70/1000 优先页，7/5000 文件。
...
完成摘要：
- Job `jichu_p4t02_v1`：5 splits / 102 pages；MinerU vlm+ch+ocr 冻结参数。
- 5/5 terminal `needs_review`（显式；fail=0）；page map 5/5 complete；页范围 1..102 连续。
- clean.v2 / structure.v1 / quality.v1 已跑通；全文/PDF 仅本地 `data/`。
- Tracked 无原文报告：`docs/superpowers/reports/2026-07-24-jichu-p4t02.{json,md}`。
- 预算后：files 12/5000，pages 172/1000（remaining 4988 / 828）。
...
验证：该书 coverage report、证候/症状表列抽查、随机页回源抽查和章节连续性。

完成摘要：
- Job `zhenduan_p4t03_v1`：4 splits / 92 pages；MinerU vlm+ch+ocr 冻结参数；batch `d49c509e-7cf4-4f46-8f26-3d4950985bce`。
- 4/4 terminal（pass=2, needs_review=2；fail=0）；page map 4/4 complete；页范围 1..92 连续。
- clean.v2 / structure.v1 / quality.v1 已跑通；全文/PDF 仅本地 `data/`。
- 表格抽查：171 tables，focus(证候/症状等)=63，focus column_skew=0，quality `bad_tables` 缺席。
- Issue codes：`empty_pages`（2 个 split 各 1 近空页，显式 needs_review）。
- Tracked 无原文报告：`docs/superpowers/reports/2026-07-24-zhenduan-p4t03.{json,md}`。
- 预算后：files 16/5000，pages 264/1000（remaining 4984 / 736）。
- 恢复点：下一串行任务 **P4-T04 中药学全量**。
...
完成报告（2026-07-24，commit `a87aef8`）：
- Job：`zhongyao_p4t04_v1` batch `0cde6505-4608-4a95-b30f-a05c6b210e73`。
- 88/88 terminal：pass 66 / needs_review 22 / fail 0；split 状态 3 pass + 1 needs_review。
- page map 4/4 complete；pages 1..88 contiguous。
- clean.v2 → structure.v1 → quality.v1。
- 表块 136；`bad_tables` 不存在；风险关键词 split 命中 药名/剂量/毒性/禁忌 = 4/4/4/4。
- Issue codes：`empty_pages`（split p0023-0044 近空页 2、6，显式 needs_review）。
- OSS 上传加固：DoH 解析真实 IP + `curl --resolve` 直连，规避本机 Fake-IP/utun 大文件中断；`submitted` 中断可 re-apply。
- Tracked 无原文报告：`docs/superpowers/reports/2026-07-24-zhongyao-p4t04.{json,md}`。
- 预算后：files 20/5000，pages 352/1000（remaining 4980 / 648）。
- 恢复点：下一串行任务 **P4-T05 方剂学全量**。
...
完成报告（2026-07-24，commit `fe1c646`）：
