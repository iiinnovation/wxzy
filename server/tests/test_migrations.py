...
ACTIVATION_REVISION = "20260727_0011"
...
ACTIVATION_TABLES = {"owner_activation_codes"}
CATALOG_TABLES = {
    "documents",
    "document_versions",
    "chapters",
...
PRE_ACTIVATION_TABLES = PRE_DAILY_PLAN_TABLES | DAILY_PLAN_TABLES
HEAD_TABLES = PRE_ACTIVATION_TABLES | ACTIVATION_TABLES
...
def test_empty_sqlite_upgrade_downgrade_upgrade(tmp_path: Path) -> None:
    ...
    assert table_names(database) == PRE_ACTIVATION_TABLES | {"alembic_version"}
    ...
    with sqlite3.connect(database) as connection:
        version = connection.execute("SELECT version_num FROM alembic_version").fetchone()
    assert version == (PROFILE_AUDIT_REVISION,)

    run_alembic(url, "downgrade", "-1")
    assert table_names(database) == DATA_TABLES | {"alembic_version"}
    with sqlite3.connect(database) as connection:
        version = connection.execute("SELECT version_num FROM alembic_version").fetchone()
    ...
    with sqlite3.connect(database) as connection:
        version = connection.execute("SELECT version_num FROM alembic_version").fetchone()
    assert version == (ACTIVATION_REVISION,)
...
def test_legacy_data_migration_refuses_destructive_downgrade(tmp_path: Path) -> None:
    ...
    run_alembic(url, "stamp", BASELINE_REVISION)
    run_alembic(url, "upgrade", "head")
    # Head may include later non-destructive revisions; target the revision that
    # would delete migrated personal learning data.
    result = run_alembic_allow_failure(url, "downgrade", ATTEMPT_REVISION)
...
def test_empty_postgres_upgrade_when_configured() -> None:
    ...
    run_alembic(url, "upgrade", "head")
    assert postgres_state(url) == (HEAD_TABLES | {"alembic_version"}, ACTIVATION_REVISION)
    assert_postgres_single_active_owner_constraint(url)

    run_alembic(url, "downgrade", "-1")
    assert postgres_state(url) == (
        PRE_ACTIVATION_TABLES | {"alembic_version"},
        STUDY_SESSION_REVISION,
    )
    ...
    run_alembic(url, "downgrade", "-1")
    assert postgres_state(url) == (DATA_TABLES | {"alembic_version"}, ATTEMPT_REVISION)
